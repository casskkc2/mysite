...
CREATE TABLE `issue` (
  ...
  `last_mod_user2_id` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '最后操作用户id(城管部门人员)',
  ...
  `lat` varchar(45) COMMENT '纬度',
  `lng` varchar(45) COMMENT '经度',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `last_mod_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `tags` text COMMENT '标签(用于搜索)',
  `start_date` date COMMENT '开始计时日期',
